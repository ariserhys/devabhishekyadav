...
const LoadingScreen: React.FC<LoadingScreenProps> = ({ onLoadingComplete }) => {
  ...
  useEffect(() => {
    if (progress >= 100) {
      // Delay the completion slightly for visual smoothness
  ...
  useEffect(() => {
    if (isComplete) {
      // Allow a bit of time for exit animation to start
      const timeout = setTimeout(() => {
        onLoadingComplete();
      }, 700);
      return () => clearTimeout(timeout);
  ...
  return (
    <AnimatePresence mode="wait">
      {!isComplete && (
        <motion.div
          className="fixed inset-0 bg-black z-50 flex flex-col items-center justify-center px-4 sm:px-6"
          initial={{ opacity: 1 }}
          exit={{ opacity: 0 }}
          transition={{ duration: 0.5, ease: 'easeInOut' }}
        >
          <div className="flex flex-col items-center justify-center mb-10 w-full max-w-[100%] sm:max-w-[90%] md:max-w-[600px]">
            {/* Apple-style "hello" SVG animation */}
            <motion.div 
            ...
            >
              <svg className="hello-svg w-full h-auto" viewBox="0 0 1230.94 414.57" preserveAspectRatio="xMidYMid meet">
                <path 
                  ...
                  style={{
                    ...
                    animation: 'animHello 3.5s linear forwards'
                  }}
                />
              </svg>
            </motion.div>
            <span className="text-sm text-blue-400 mt-6 text-center">I'm Abhishek Yadav • Full Stack Developer</span>
          </div>
